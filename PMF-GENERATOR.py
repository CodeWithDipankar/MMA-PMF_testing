...
class Controller:
    def run_main_logic(self, core_path, custom_path, update_ui, on_done):
        try:
            ...
            for j in range(noOfNewWeeks):
                PMF.iloc[:,len(PMF.columns)-noOfNewWeeks+j]=PMF.iloc[:,len(PMF.columns)-noOfNewWeeks-1]
            ...
            ## Cross Check TAB ##
            CROSS_CHECK = pd.concat([coreFrameWork,(MATCHBACK_WEEKLY.iloc[:,2:].div(CORE_WEEKLY.iloc[:,2:],fill_value=1))],axis=1)
            ...
            ADJ_WEEK = pd.concat([coreFrameWork,(PMF[PMF.columns[2:PMF.shape[1]-2]].multiply(CORE_WEEKLY[CORE_WEEKLY.columns[2:CORE_WEEKLY.shape[1]-2]]))],axis=1)
